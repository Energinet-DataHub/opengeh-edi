...
public class IncomingMessageReceiverTests : TestBase, IAsyncLifetime
{
        ...
        Authenticated_user_must_hold_the_role_type_as_specified_in_message_unless_Ddm_Mdr_hack_applicable()
    {
        ...
        authenticatedActor.SetAuthenticatedActor(
            new ActorIdentity(ActorNumber.Create("0192837465019"), Restriction.None, ActorRole.GridAccessProvider, null, ActorId));
        ...
        // RESET TEST CLASS!
        authenticatedActor.SetAuthenticatedActor(
            new ActorIdentity(ActorNumber.Create("1234567890123"), Restriction.None, ActorRole.FromCode("DDQ"), null, ActorId));
    }
    ...
    {
        var authenticatedActor = GetService<AuthenticatedActor>();
        authenticatedActor.SetAuthenticatedActor(new ActorIdentity(ActorNumber.Create(knownSenderId), restriction: Restriction.None,  ActorRole.FromCode(knownSenderRole), null, ActorId));
