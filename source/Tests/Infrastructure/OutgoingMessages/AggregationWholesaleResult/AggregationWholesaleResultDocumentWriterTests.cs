...
public class AggregationWholesaleResultDocumentWriterTests : IClassFixture<DocumentValidationFixture>
{
    private readonly DocumentValidationFixture _documentValidation;
    private readonly IMessageRecordParser _parser;
    private readonly TimeSeriesBuilder _timeSeries;

    public AggregationWholesaleResultDocumentWriterTests(DocumentValidationFixture documentValidation)
    {
    ...
    private Task<Stream> CreateDocument(TimeSeriesBuilder resultBuilder, DocumentFormat documentFormat)
    {
        ...
        {
            return new AggregationWholesaleResultEbixDocumentWriter(_parser).WriteAsync(
                documentHeader,
                new[] { records, });
        }
        else if (documentFormat == DocumentFormat.Xml)
        {
            return new AggregationWholesaleResultXmlDocumentWriter(_parser).WriteAsync(
                documentHeader,
                new[] { records, });
        }
        else
        {
            return new AggregationWholesaleResultJsonDocumentWriter(_parser).WriteAsync(
                documentHeader,
                new[] { records, });
        }
    }

    private IAssertAggregationWholesaleResultDocument AssertDocument(Stream document, DocumentFormat documentFormat)
    {
        if (documentFormat == DocumentFormat.Ebix)
        {
            var assertEbixDocument = AssertEbixDocument.Document(document, "ns0", _documentValidation.Validator);
            return new AssertAggregationWholesaleResultEbixDocument(assertEbixDocument);
        }
        else if (documentFormat == DocumentFormat.Xml)
        {
            var assertXmlDocument = AssertXmlDocument.Document(document, "cim", _documentValidation.Validator);
            return new AssertAggregationWholesaleResultXmlDocument(assertXmlDocument);
        }
        else
        {
            return new AssertAggregationWholesaleResultJsonDocument(document);
        }
    }
}
