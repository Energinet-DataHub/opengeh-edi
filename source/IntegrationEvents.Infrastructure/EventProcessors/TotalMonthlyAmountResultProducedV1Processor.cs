...
public class TotalMonthlyAmountResultProducedV1Processor : IIntegrationEventProcessor
{
    ...
    public async Task ProcessAsync(IntegrationEvent integrationEvent, CancellationToken cancellationToken)
    {
        ...
        if (!await _featureManager.UseTotalMonthlyAmountResultProducedAsync().ConfigureAwait(false))
        {
            return;
        }
        ...
        var isHandledByCalculationCompletedEvent = await totalMonthlyAmountResultProducedV1.CalculationType
