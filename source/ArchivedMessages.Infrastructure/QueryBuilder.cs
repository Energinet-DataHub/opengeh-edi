...
internal sealed class QueryBuilder
{
    ...
    public QueryInput BuildFrom(GetMeteringPointMessagesQuery query)
    {
        ...
        // TODO: Much of this is duplicated, consider refactoring
        if (query.SenderNumber is not null && query.ReceiverNumber is not null)
        {
            AddFilter(
                "(SenderNumber=@SenderNumber OR ReceiverNumber=@ReceiverNumber)",
    ...
    private string BuildStatement(GetMessagesQuery query)
    {
        ...
        if (query.IncludeRelatedMessages == true && query.MessageId is not null)
        {
            ...
            // Case 1 and 2 may be solved by joining every request onto a response (t1.RelatedToMessageId = t2.MessageId)
            ...
            // For case 2: Table 2 would consist of the response you searched for, all other responses which has a reference to the same request and the request itself (containing duplications)
