...
{
    public abstract class JsonDocumentWriter : DocumentWriter
    {
        private readonly DocumentDetails _documentDetails;

        protected JsonDocumentWriter(DocumentDetails documentDetails, IMessageRecordParser parser)
            : base(parser)
        {
        ...
        public override async Task<Stream> WriteAsync(MessageHeader header, IReadOnlyCollection<string> marketActivityRecords, IReadOnlyCollection<string> originalData)
        {
            ...
            JsonHeaderWriter.Write(header, _documentDetails.DocumentType, _documentDetails.TypeCode, _documentDetails.ReasonCode, writer);
