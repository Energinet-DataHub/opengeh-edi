...
{
    public abstract class CimDocumentWriter : DocumentWriter
    {
        private readonly DocumentDetails _documentDetails;
        private readonly string? _reasonCode;

        protected CimDocumentWriter(DocumentDetails documentDetails, IMessageRecordParser parser, string? reasonCode = null)
            : base(parser)
        {
        ...
        public override async Task<Stream> WriteAsync(MessageHeader header, IReadOnlyCollection<string> marketActivityRecords, IReadOnlyCollection<string> originalData)
        {
            ...
            await WriteHeaderAsync(header, DocumentDetails, writer).ConfigureAwait(false);
        ...
        internal static async Task WriteEndAsync(XmlWriter writer)
        {
            await writer.WriteEndElementAsync().ConfigureAwait(false);
            writer.Close();
        }

        internal Task WriteHeaderAsync(MessageHeader header, DocumentDetails documentDetails, XmlWriter writer)
        {
            return XmlHeaderWriter.WriteAsync(writer, header, documentDetails, ReasonCode);
        }
