...
public class RetrieveAggregationResultHandler : IRequestHandler<RetrieveAggregationResult, Unit>
{
    ...
    public async Task<Unit> Handle(RetrieveAggregationResult request, CancellationToken cancellationToken)
    {
        ...
        var aggregationResult = await _aggregationResults.ProductionResultForAsync(request.ResultId, request.GridArea, new Domain.Transactions.Aggregations.Period(request.Period.Start, request.Period.End)).ConfigureAwait(false);
