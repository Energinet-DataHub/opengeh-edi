...
namespace OutgoingMessages.Application.Tests.MarketDocuments.Asserts;
...
public class AssertEbixDocument
{
    ...
    public AssertEbixDocument HasValue(string xpath)
    {
        return HasValueWithAttributes(xpath);
    }
    ...
    public AssertEbixDocument HasValueWithAttributes(string xpath, params AttributeNameAndValue[] attributes)
    {
        ArgumentNullException.ThrowIfNull(xpath);
        ArgumentNullException.ThrowIfNull(attributes);
        Assert.True(!string.IsNullOrWhiteSpace(_document.Root?.XPathSelectElement(EnsureXPathHasPrefix(xpath), _xmlNamespaceManager)?.Value));
    ...
    public async Task<AssertEbixDocument> HasValidStructureAsync(DocumentType type, string version = "0.1")
    {
        ...
        validationResult.ValidationErrors
            // We ignore the validation error message for transaction id, since we currently creating document which is not valid on this field
            .Where(
                x => !x.Contains(
                    "The 'un:unece:260:data:EEM-DK_AggregatedMeteredDataTimeSeries:v3:Identification'",
                    StringComparison.InvariantCultureIgnoreCase))
            .Where(
                x => !x.Contains(
                    "The 'un:unece:260:data:EEM-DK_NotifyAggregatedWholesaleServices:v3:Identification'",
                    StringComparison.InvariantCultureIgnoreCase))
            .Should()
            .BeEmpty();
