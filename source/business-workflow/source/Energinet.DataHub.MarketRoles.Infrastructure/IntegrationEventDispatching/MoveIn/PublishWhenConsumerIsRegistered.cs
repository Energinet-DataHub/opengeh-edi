...
{
    public class PublishWhenConsumerIsRegistered : INotificationHandler<ConsumerRegistered>
    {
        private readonly IOutbox _outbox;
        private readonly IOutboxMessageFactory _outboxMessageFactory;

        public PublishWhenConsumerIsRegistered(IOutbox outbox, IOutboxMessageFactory outboxMessageFactory)
        {
            _outbox = outbox ?? throw new ArgumentNullException(nameof(outbox));
            _outboxMessageFactory = outboxMessageFactory ?? throw new ArgumentNullException(nameof(outboxMessageFactory));
        }

        public Task Handle(ConsumerRegistered notification, CancellationToken cancellationToken)
        {
            if (notification == null) throw new ArgumentNullException(nameof(notification));

            var integrationEvent = new ConsumerRegisteredIntegrationEvent();
