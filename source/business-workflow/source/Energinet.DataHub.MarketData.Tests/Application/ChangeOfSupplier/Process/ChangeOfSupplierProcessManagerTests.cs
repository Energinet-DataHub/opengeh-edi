...
namespace Energinet.DataHub.MarketData.Tests.Application.ChangeOfSupplier.Process
{
    ...
    public class ChangeOfSupplierProcessManagerTests
    {
        ...
        [Fact]
        public void EnergySupplierChangeIsRegistered_WhenStateIsNotStartet_ConfirmationMessageIsSend()
        {
        ...
        public void ConfirmationMessageIsDispatched_WhenStateIsAwaitingConfirmationMessage_MasterDataDetailsIsSend()
        {
            ...
            var command =
                processManager.CommandsToSend.First(c => c.Command is SendMasterDataDetails).Command as
                    SendMasterDataDetails;
        ...
        [Fact]
        public void MasterDataDetailsIsDispatched_WhenStateIsAwaitMasterDataDetailsDispatch_CurrentSupplierInformationIsSend()
        {
            var processManager = Create();
            var (processId, gsrnNumber, effectiveDate) = CreateTestValues();
            ...
            processManager.When(new EnergySupplierChangeRegistered(gsrnNumber, processId, effectiveDate));
            processManager.When(new ConfirmationMessageDispatched(processId.Value));
            processManager.When(new MasterDataDetailsDispatched(processId.Value));
        ...
        [Fact]
        public void MasterDataDetailsIsDispatched_WhenStateIsNotAwaitMasterDataDetailsDispatch_ThrowException()
        {
            var processManager = Create();
            var (processId, gsrnNumber, effectiveDate) = CreateTestValues();

            Assert.Throws<InvalidProcessManagerStateException>(() => processManager.When(new MasterDataDetailsDispatched(processId.Value)));
        }
        ...
        public void CurrentSupplierIsNotified_WhenStateIsNotAwaitingCurrentSupplierNotification_ThrowException()
        {
            ...
            Assert.Throws<InvalidProcessManagerStateException>(() => processManager.When(new CurrentSupplierNotified(processId.Value)));
        }
