    ...
    IReadOnlyCollection<IndividualMissingMeasurements> individualMissingMeasurements)
{
    ...
    public ValueObjectWithScheme ReceiverNumber { get; init; } = ValueObjectWithScheme.Create(ActorNumber.Create(receiverId).IsEic() ? "A01" : "A10", receiverId);
    ...
    public ValueObjectWithScheme SenderNumber { get; init; } = ValueObjectWithScheme.Create(ActorNumber.Create(senderId).IsEic() ? "A01" : "A10", senderId);
