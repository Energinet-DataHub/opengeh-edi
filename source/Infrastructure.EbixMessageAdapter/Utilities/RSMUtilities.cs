...
using Energinet.DataHub.EDI.Infrastructure.EbixMessageAdapter.Exceptions;
...
#nullable disable
        ...
        {
            ArgumentNullException.ThrowIfNull(rsmXmlDoc, nameof(rsmXmlDoc));
...
#nullable disable
        ...
        private static XmlDocument ConvertXmlSchemeVersion(XmlDocument srcRsmDoc, SchemeVersion srcSchVer, SchemeVersion tarSchVer)
        {
            ...
            var tarDocXmlNs = srcDocXmlNs.Replace(srcVer, tarVer, StringComparison.InvariantCultureIgnoreCase);
            var srcDocAsStr = RSMUtilities.ConvertToString(srcRsmDoc);
            var tarDocAsStr = srcDocAsStr.Replace(srcDocXmlNs, tarDocXmlNs, StringComparison.InvariantCultureIgnoreCase);
