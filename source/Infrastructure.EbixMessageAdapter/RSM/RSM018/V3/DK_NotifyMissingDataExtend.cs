...
namespace Energinet.DataHub.EDI.Infrastructure.EbixMessageAdapter.RSM.RSM018.V3.Notify
{
    public partial class DK_NotifyMissingDataType : RsmDocument
    {
        public override int PayloadCount
        {
            get { return PayloadMissingDataRequest.Length; }
        }

        public override void AddPayload(object payload)
        {
            var payloads = PayloadMissingDataRequest.ToList();
            payloads.Add((MissingDataRequest)payload);
            PayloadMissingDataRequest = payloads.ToArray();
        }
        ...
        {
            return PayloadMissingDataRequest[payloadIndex].RequestPeriod;
        }

        public override object GetPayload(int payloadIndex)
        {
            return PayloadMissingDataRequest[payloadIndex];
        }

        public override string GetPayloadMessageId(int payloadIndex)
        {
            return PayloadMissingDataRequest[payloadIndex].Identification;
        }
        ...
        {
            PayloadMissingDataRequest = payloads.Cast<MissingDataRequest>().ToArray();
        }
    }
}
