...
namespace Energinet.DataHub.SoapAdapter.Application
{
    ...
    public class SendMessageService : ISendMessageService
    {
        ...
        private async Task<Stream> ConvertToInternalAsync(Stream stream, Context context, string correlationId)
        {
            var converter = _requestConverterFactory.GetConverter(context.RsmDocumentType.Value);

            var output = new MemoryStream();
            await converter.ConvertAsync(stream, output, correlationId).ConfigureAwait(false);
            return output;
        }
