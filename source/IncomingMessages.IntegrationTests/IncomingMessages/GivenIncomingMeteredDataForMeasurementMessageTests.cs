...
public class GivenIncomingMeteredDataForMeasurementMessageTests : IncomingMessagesTestBase
{
    ...
    public async Task When_BusinessProcessIsIncorrect_Then_ResultContainExceptedValidationError()
    {
        ...
        var (_, result) = await ParseMessageAsync(message.Stream);
    ...
    [Theory]
    [InlineData("E23")]
    [InlineData("D42")] //Only for Ebix
    public async Task When_ProcessTypeIsAllowed_Then_ValidationSucceed(string allowedProcessType)
    {
        var message = MeteredDataForMeasurementPointBuilder.CreateIncomingMessage(
            DocumentFormat.Ebix,
            _actorIdentity.ActorNumber,
        ...
        Stream message)
    {
        var messageParser = await _marketMessageParser.ParseAsync(
            ...
            CancellationToken.None);
        return (IncomingMessage: (MeteredDataForMeasurementPointMessage?)messageParser.IncomingMessage, ParserResult: messageParser);
    }
